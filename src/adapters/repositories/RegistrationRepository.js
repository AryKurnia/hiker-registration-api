...
class RegistrationRepository extends IRegistrationRepository {
  ...
  async create(registration) {
    ...
    const queryInsert = `INSERT INTO ${this.tableName} (id_pendaftaran, id_pendaki, tanggal_pendakian, status, note) VALUES (NULL, ?, ?, ?, ?)`;
    const valuesInsert = [registration.hikerId, registration.hikingDate,
      registration.status, registration.note];
    ...
    try {
      ...
      const querySelect = 'SELECT nama_lengkap AS name, nik FROM tbl_pendaki WHERE id_pendaki = ?';
      const dataSelect = await this.database.query(querySelect, [registration.hikerId]);

      const result = {
        registeredId,
        hikerName: dataSelect[0][0].name,
        hikerNik: dataSelect[0][0].nik,
      };
